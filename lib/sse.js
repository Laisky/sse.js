...
var SSE = function (url, options) {
  ...
  this._onStreamProgress = function(e) {
    ...
    // Add the headers of the response to the event object
    //
    // if this._onStreamLoaded is not called, we still want to have the headers,
    // so we parse them here as well
    if (this.xhr && this.xhr.getAllResponseHeaders()
      && Object.keys(this.responseHeaders).length === 0) {
      const headerPairs = this.xhr.getAllResponseHeaders().split('\r\n');
      for (const headerPair of headerPairs) {
        const [key, ...valueParts] = headerPair.split(':');
        const value = valueParts.join(':').trim();
        const lowerCaseKey = key.trim().toLowerCase();
        // Ensure the header value is always an array and unique
        if (!this.responseHeaders[lowerCaseKey]) {
          this.responseHeaders[lowerCaseKey] = [];
        }
        if (!this.responseHeaders[lowerCaseKey].includes(value)) {
          this.responseHeaders[lowerCaseKey].push(value);
        }
      }
    }
  ...
  this._onStreamLoaded = function(e) {
    // Add the headers of the response to the event
    //
    // _onStreamLoaded may not be called in some cases.
    if (this.xhr && this.xhr.getAllResponseHeaders()) {
      const headerPairs = this.xhr.getAllResponseHeaders().split('\r\n');
      for (const headerPair of headerPairs) {
        const [key, ...valueParts] = headerPair.split(':');
  ...
  this._parseEventChunk = function(chunk) {
    ...
    event.headers = this.responseHeaders;  // Add the headers of the response to the event object
